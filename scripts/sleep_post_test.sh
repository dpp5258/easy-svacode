#!/usr/bin/env bash
# =====================================================================
# 睡岗检测联调测试脚本(以算法分支契约为准:sleep 规则 + hdThreshold)—— 分析器角色工作台工具
# 用法:
#   bash scripts/sleep_post_test.sh <视频文件或RTSP/HTTP流地址>
# 可选环境变量(阈值档位):
#   DURATION_MS=8000     规则 thresholdMs(持续低头时长,算法契约建议:真实8000/演示2000)
#   HD_RATIO=0.12        规则 hdThreshold(头肩距离比阈值,算法契约默认0.12)
#   DETECT_FPS=8         算法检测帧率
#   RUN_SECONDS=20       观察时长
...
#   2) 后端已含 sleep→SVA_SLEEP/睡岗告警 映射(integration 分支 HWaringController 补丁)
...
BODY=$(cat <<JSON
{
  ...
  "geometryConfig": {
    ...
    "behaviorRules": [{
      ...
      "behaviorType": "sleep", "enabled": true,
      "geometryId": "region_1",
      "thresholdMs": $DURATION_MS, "hdThreshold": $HD_RATIO,
...
while [ $SECONDS -lt $END ]; do
    ...
    ALARMS=$(grep -c "behavior_type\" : \"sleep\"" /opt/SVA/server/log.out 2>/dev/null || echo 0)
    echo "[$((END - SECONDS))s 剩余] detectFrameSent=$SENT detectEventSent=$EVSENT 上报sleep告警次数≈$ALARMS"
    sleep 5
done

echo "== 4/4 检查分析器日志中的睡岗命中/上报 =="
grep -E "SVA_SLEEP|睡岗|behavior_type" /opt/SVA/server/log.out | tail -10 || echo "(日志无睡岗命中——检查视频内容或阈值档位)"
